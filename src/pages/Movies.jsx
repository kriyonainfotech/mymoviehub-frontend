...
const Movies = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        setMovies(movRes.data.filter(m => m.type === 'movie'));
        
        const movieBanner = banRes.data.find(b => (b.pages && b.pages.includes('movies')) || b.page === 'movies');
  ...
  // Filter categories that should appear on movie page
  const moviePageCategories = categories.filter(c => c.sections && c.sections.includes('movie'));
  ...
  return (
    <>
      ...
      <div className="-mt-4 relative z-10">
        {moviePageCategories.map((category, index) => {
